...
def main() -> int:
    ...
    try:
        ...
        L.append("")
        L.append("-" * 79)
        L.append("SYNC SERVER")
        L.append("-" * 79)
        L.append("  This test STARTS ITS OWN ephemeral sync server automatically (python -m")
        L.append("  anki.syncserver on a free loopback port) and stops it at the end - so")
        L.append("  reproducing 7b needs NO manual server step; one command does everything.")
        L.append("")
        L.append("  To run a STANDING server instead (e.g. a manual desktop demo):")
        L.append("      ./tools/gmat-sync-server.sh")
        L.append("  Defaults: user gmat:wiz, http://127.0.0.1:27811/, data in out/syncserver")
        L.append("  (override via GMAT_SYNC_USER / GMAT_SYNC_PORT / GMAT_SYNC_BASE). Point")
        L.append("  clients at that URL; the iOS simulator reaches the host's 127.0.0.1.")
        L.append("")
        L.append("-" * 79)
        L.append("SETUP")
        L.append("-" * 79)
